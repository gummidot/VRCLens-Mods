...
public static class VRCLensShaderPatcher
{
    ...
    /// </summary>
    public static Shader PatchShader(bool enableLowerMinFocus, bool enableManualFocusAssist, bool enableGhostFX = false, bool enableChromaticAberration = false, bool enableFilmGrain = false, bool enableDepthFog = false, bool enableTiltShift = false, bool enableFisheye = false, bool enableColorGrading = false, bool enableLetterbox = false)
    {
        ...
        // Validate Letterbox anchors if that feature is enabled
        if (enableLetterbox)
        {
            var lbAnchors = GetLetterboxAnchors();
            List<string> missingLB = new List<string>();
            foreach (var anchor in lbAnchors)
            {
                if (!content.Contains(anchor.SearchString))
                    missingLB.Add($"  - {anchor.Description}: \"{anchor.SearchString}\"");
            }
            if (missingLB.Count > 0)
            {
                string errorMsg = $"Cannot patch shader -- expected Letterbox anchor lines not found.\n" +
                    $"VRCLens may have been updated with incompatible changes.\n\n" +
                    $"Missing anchors:\n{string.Join("\n", missingLB)}";
                Debug.LogError($"{LOG_PREFIX} {errorMsg}");
                return null;
        ...
        // Step 1i: Apply Letterbox insertions
        if (enableLetterbox)
        {
            content = ApplyLetterboxInsertions(content);
        }
        ...
        content = RenameShader(content, enableLowerMinFocus, enableManualFocusAssist, enableGhostFX, enableChromaticAberration, enableFilmGrain, enableDepthFog, enableTiltShift, enableFisheye, enableColorGrading, enableLetterbox);
        ...
        if (enableLetterbox) enabledMods.Add("Letterbox");
    ...
    public static void VerifyPatcher()
    {
        ...
        // Validate Letterbox anchors
        var lbAnchorsV = GetLetterboxAnchors();
        List<string> missingLBV = new List<string>();
        foreach (var anchor in lbAnchorsV)
        {
            if (!originalContent.Contains(anchor.SearchString))
                missingLBV.Add($"  - {anchor.Description}: \"{anchor.SearchString}\"");
        }
        if (missingLBV.Count > 0)
        {
            string msg = $"Letterbox validation FAILED -- missing anchors:\n{string.Join("\n", missingLBV)}\n\n" +
                "VRCLens may have been updated. See maintenance steps in the patcher source code.";
            Debug.LogError($"{LOG_PREFIX} {msg}");
            EditorUtility.DisplayDialog("Verify Patcher -- FAILED", msg, "OK");
            return;
        }
        messages.Add($"All {lbAnchorsV.Count} Letterbox anchors found.");
        ...
        patched = ApplyLetterboxInsertions(patched);
    ...
    {
        Shader shader = PatchShader(true, true, true, true, true, true, true, true, true, true);
        if (shader != null)
        {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // Letterbox insertion logic
    // ═══════════════════════════════════════════════════════════════════

    private static readonly string BLOCK_LETTERBOX_PROPERTIES = @"
		// VRCLens_Custom BEGIN - Letterbox Properties
		[Header(Letterbox)]
		_LetterboxAspect (""Letterbox Aspect (W over H)"", Range(0.5, 3.0)) = 1.7777778
		// VRCLens_Custom END";

    private static readonly string BLOCK_LETTERBOX_UNIFORMS = @"
			// VRCLens_Custom BEGIN - Letterbox Uniforms
			uniform float _LetterboxAspect;
			// VRCLens_Custom END";

    // Compare _LetterboxAspect against _AspectRatio (the camera sensor aspect),
    // NOT _ScreenParams.x/y. The latter is the render-target size and is
    // hardcoded to 1024x512 (aspect 2.0) during in-headset preview, so using
    // it would mis-position the bars whenever the sensor isn't 16:9 or in
    // preview mode. _AspectRatio is the W/H of the captured image and is
    // what users mean when they pick a target ratio to crop to.
    //
    // Outer gate '_LetterboxAspect > 0.01' is the 'no letterbox' sentinel:
    // when all preset toggles are off, the animator drives _LetterboxAspect
    // to 0, which would otherwise be interpreted as a 0:1 aspect (full
    // pillarbox = blackout). We can't pick a non-zero default since we
    // don't know the user's sensor aspect, so treat 0 as 'disabled'.
    private static readonly string BLOCK_LETTERBOX_DITHERING = @"
				// VRCLens_Custom BEGIN - Letterbox bars
				if (_LetterboxAspect > 0.01) {
					if (abs(_LetterboxAspect - _AspectRatio) > 0.01) {
						if (_LetterboxAspect > _AspectRatio) {
							float lbVisible = _AspectRatio / _LetterboxAspect;
							float lbBar = (1.0 - lbVisible) * 0.5;
							if (i.uv.y < lbBar || i.uv.y > 1.0 - lbBar) col.rgb = 0;
						} else {
							float lbVisible = _LetterboxAspect / _AspectRatio;
							float lbBar = (1.0 - lbVisible) * 0.5;
							if (i.uv.x < lbBar || i.uv.x > 1.0 - lbBar) col.rgb = 0;
						}
					}
				}
				// VRCLens_Custom END";

    private static List<AnchorInfo> GetLetterboxAnchors()
    {
        return new List<AnchorInfo>
        {
            new AnchorInfo(ANCHOR_PROPERTIES, "Properties: _DoFStrength line"),
            new AnchorInfo(ANCHOR_PASS6_UNIFORMS, "Pass 2: _FocusDistance uniform"),
            new AnchorInfo(ANCHOR_DITHERING, "Pass 2: dithering line"),
        };
    }

    private static string ApplyLetterboxInsertions(string content)
    {
        var lines = new List<string>(content.Split(new[] { "\r\n", "\n" }, StringSplitOptions.None));
        int insertions = 0;

        // Letterbox bars -- after dithering, last operation in Pass 2 so bars
        // cover everything. Pure black write (col.rgb = 0) so order vs other
        // ANCHOR_DITHERING insertions does not matter.
        insertions += InsertAfterLine(lines, ANCHOR_DITHERING, BLOCK_LETTERBOX_DITHERING, "Letterbox bars");

        // Letterbox uniforms in Pass 2 -- after second _FocusDistance uniform
        insertions += InsertAfterLine(lines, ANCHOR_PASS6_UNIFORMS, BLOCK_LETTERBOX_UNIFORMS, "Letterbox uniforms", false, 2);

        // Letterbox property -- after _DoFStrength line
        insertions += InsertAfterLine(lines, ANCHOR_PROPERTIES, BLOCK_LETTERBOX_PROPERTIES, "Letterbox property");

        Debug.Log($"{LOG_PREFIX} Applied {insertions} Letterbox insertion sites");
    ...
    /// </summary>
    private static string RenameShader(string content, bool enableLowerMinFocus, bool enableManualFocusAssist, bool enableGhostFX = false, bool enableChromaticAberration = false, bool enableFilmGrain = false, bool enableDepthFog = false, bool enableTiltShift = false, bool enableFisheye = false, bool enableColorGrading = false, bool enableLetterbox = false)
    {
        ...
        if (enableLetterbox) parts.Add("Letterbox");
